...
const Users = {
    create: async (data) => {
        const sql = 'INSERT INTO users (name, password, mobile, email, address, dob, roleId, isActive, created_at, updated_at) VALUES (?, ?, ?, ?,?, ?, ?, ?, NOW(), NOW())';
        try {
            const [results] = await db.execute(sql, [data.name, data.password, data.mobile, data.email, data.address, data.dob, data.roleId, data.isActive]);
    ...
    update: async (id, data) => {
        const sql = 'UPDATE users SET name = ?, password = ?, roleId = ?, isActive = ?,  mobile = ?,  email = ?,  address = ?,  dob = ?, updated_at = NOW() WHERE id = ?';
        try {
            const [results] = await db.execute(sql, [data.name, data.password, data.roleId, data.isActive, data.mobile, data.email, data.address, data.dob, id]);
